...
set /p TtempPass=<conf\adv_backup\advTempPass
...
if exist "%TsrcPath%" (
	if exist "%TdestPath%" (
		call "%TinstPath%\7z\7za" a %TcompType% %TcompLvl% MigrateToGDrive_%today%_%TrndmStrg%.%TcompExt% "%TsrcPath%\*" -p%TtempPass%
		if %ERRORLEVEL% == 0 goto :next
		echo err>> "log/lastResult"
		echo. >> "log/adverr"
		echo # MigrateToGDrive v1.1 >> "log/adverr"
		echo Compress Result				: Error >> "log/adverr"
		echo Compress Time				: %date% - %time% >> "log/adverr"
		echo Compress Filename			: MigrateToGDrive_%today%_%TrndmStrg%.%TcompExt% >> "log/adverr"
		...
		echo # MigrateToGDrive v1.1 >> "log/advlog"
